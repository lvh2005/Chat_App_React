...
import { FaAngleLeft, FaPlus, FaPhoneAlt, FaVideo, FaTrash, FaUser } from "react-icons/fa";
...
const MessagePage = () => {
      ...
      <header className='sticky top-0 h-16 bg-white flex justify-between items-center px-4'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-sm'>{dataUser.online ? <span className='text-primary'>online</span> : <span className='text-slate-400'>offline</span>}</p>
          </div>
        </div>
        <div className='flex items-center gap-4'>
          <button className='hover:text-primary'><FaPhoneAlt size={20} /></button>
          <button className='hover:text-primary'><FaVideo size={20} /></button>
          <button onClick={() => setOpenMenu(!openMenu)} className='hover:text-primary'>
            <HiDotsVertical size={24} />
          </button>
          {openMenu && (
            <div className='absolute right-4 top-16 bg-white shadow-md rounded p-2 w-48'>
              <button className='flex items-center gap-2 p-2 hover:bg-gray-100 w-full'>
                <FaUser /> <span>Xem thông tin</span>
              </button>
              <button className='flex items-center gap-2 p-2 hover:bg-gray-100 w-full'>
                <FaTrash /> <span>Xóa cuộc trò chuyện</span>
              </button>
            </div>
          )}
        </div>
                            ...
                            <div className='w-full relative'>
                            {
                                msg?.imageUrl && (
                                  <img 
                                    ...
                                    onClick={() => openImageInModal(msg.imageUrl)}
                                  />
                                )
                              }
          ...
          <section className='h-16 bg-white flex items-center px-4'>
              ...
              <form className='h-full w-full flex gap-2' onSubmit={handleSendMessage}>
                ...
                className='py-1 px-4 outline-none w-full h-full border border-gray-300 rounded-full'
                value={message.text}
  ...
  <div className='fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50'>
    <div className='relative'>
      <button
        className='absolute top-2 right-2 text-white bg-gray-800 p-1 rounded-full hover:bg-red-500'
        onClick={() => setOpenImageModal(false)}
      >
        <IoClose size={24} />
      </button>
      <img src={modalImageUrl} alt="Zoomed" className='max-w-full max-h-screen object-contain' />
    </div>
  </div>
)}
